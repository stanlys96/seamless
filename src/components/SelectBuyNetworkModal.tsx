...
interface Props {
  networkModal: any;
  setNetworkModal: (param1: any) => void;
  setCurrentNetwork: (param1: any) => void;
}

export const SelectBuyNetworkModal = ({
  networkModal,
  setNetworkModal,
  setCurrentNetwork,
}: Props) => {
              ...
              <div className="relative border-gray p-4">
                <div>
                  <div className="noScrollbar -mx-2 flex overflow-x-auto flex-wrap">
                    {resultData &&
                      resultData.map((result: any, idx: any) => (
                        <button
                          onClick={async (e) => {
                            e.preventDefault();
                            setNetworkModal(false);
                        ...
                        >
                          <div className="skt-w rounded-full overflow-hidden h-6 w-6 mr-1.5">
                            <img
                              src={
                                chainData.find(
                                  (data) => data.chainId === result.chainId
                                )?.imgUrl
                              }
                              width="100%"
                              height="100%"
