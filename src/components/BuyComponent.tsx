    ...
    <div className="the-container relative mb-5">
      <div className="w-full flex flex-col justify-center items-center">
        ...
        >
          ...
          >
            ...
            {feesOpen && (
              <div className="third-item flex gap-x-2">
                ...
                />
                <span className="text-secondGray">
                  {currentFixedFee.toPrecision(2)} {currentSelectedToken?.name}{" "}
                  Processing Fee
                </span>
              </div>
            )}
          ...
          <div className="flex justify-between bg-container md:px-[24px] px-[10px] py-[10px] rounded-[12px] ">
            <div className="flex flex-col gap-y-2">
              <div className="flex items-center text-white">
                <span className="text-[16px]">Receiving in </span>
                <button
                  onClick={() => {
                    setNetworkModal(true);
                  }}
                  ...
                  <span className="flex items-center">
                    <div className="relative flex h-fit w-fit">
                      <div className="skt-w rounded-full overflow-hidden w-5 h-5 sm:w-6 sm:h-6">
                        <img
                          src={
                            currentSelectedNetwork?.imgUrl ?? "/img/Ether.svg"
                          }
                          width="100%"
                          height="100%"
                        />
                    ...
                    <span className="skt-w ml-1 -mb-0.5 font-medium text-white text-[16px]">
                      {currentSelectedNetwork?.name ?? "Ethereum"}
                    </span>
                    <AiOutlineArrowDown />
                  </span>
                </button>
              ...
              <CurrencyInput
                ...
                onValueChange={(value, name) => {
                  ...
                  if (data) {
                    const currentPrice = data.data[0].current_price;
                    const idr = (
                      currentPrice * parseFloat(value ?? "0")
                    ).toFixed(2);
                    const thisFee = parseFloat(
                      (
                        currentFixedFee +
                    ...
                    setExchangeFee(
                      (parseFloat(value ?? "0") * 0.008).toFixed(2)
                    );
                    setFee(thisFee);
              ...
              <p className="text-cute">
                ...
                {`${
                  currentSelectedToken?.native
                    ? parseFloat(nativeBalance?.data?.formatted ?? "0").toFixed(
                        5
                      )
                    : parseFloat(tokenBalance?.data?.formatted ?? "0").toFixed(
                        5
                      )
                }`}
              </p>
          ...
          <button
            disabled={loading || isCheckingBankAccount || liquidityUnavailable}
            onClick={async (e) => {
              e.preventDefault();
              coingeckoMutate();
              try {
