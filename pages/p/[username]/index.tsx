...
export default function HomePage() {
  ...
  const { data: userWalletsData } = useSWR("/api/user-wallets", fetcherStrapi);

  const userWalletsResult = userWalletsData?.data?.data;
  const currentUsername = router?.query?.username as string;
  const findUsername = userWalletsResult?.find(
    (userWallet: any) =>
      userWallet?.attributes?.username?.toLowerCase() ===
      currentUsername?.toLowerCase()
  );

  const { data: historyData } = useSWR(
    `/api/wallet-accounts?filters[wallet_address][$eq]=${findUsername?.attributes?.wallet_address}`,
    fetcherStrapi
  );
  ...
  return (
    <MainLayout>
      {findUsername ? (
          ...
          <div className="w-full flex flex-col justify-center items-center">
            ...
            >
              ...
              <div className="bg-container flex justify-between items-center md:px-[24px] px-[10px] py-[10px] rounded-[12px]">
                ...
                <p className="text-white">@{currentUsername}</p>
              </div>
        ...
        <div>
          <p className="flex justify-center items-center h-[40vh]">
